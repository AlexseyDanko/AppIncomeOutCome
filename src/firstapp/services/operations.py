...
from ..models.operations import OperationType, OperationCreate, OperationUpdate
    ...
    def _get(self, user_id: int,  operation_id: int):  # -> tables.Operation
        ...
        if not operation:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail={'Not found id': 'Please check your text'}
                                )
        return operation

    def get_list(self, user_id: int, kind: Optional[OperationType] = None):  # -> List[tables.Operation]
        query = (
            self.session
            .query(tables.Operation)
